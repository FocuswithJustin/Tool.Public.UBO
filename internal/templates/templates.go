...
Files in this directory:
  client_wg.conf         — WireGuard client config
  client_auth_ed25519    — SSH key for Dropbear
  dropbear_host_key.pub  — Pinned Dropbear host key
...
2. When the server is in the initramfs stage, run:

   ubo unlock --config {{.ConfigPath}}

   ubo will bring up the WireGuard tunnel automatically (no root required),
   SSH to Dropbear, and run cryptroot-unlock.
...
----------------------
   ...
   ubo unlock change --config {{.ConfigPath}}
